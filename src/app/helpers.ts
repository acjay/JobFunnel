import { BlockObjectResponse, ChildDatabaseBlockObjectResponse, PartialBlockObjectResponse, RichTextItemResponse } from "@notionhq/client/build/src/api-endpoints";

export function isChildDatabase(response: BlockObjectResponse | PartialBlockObjectResponse): response is ChildDatabaseBlockObjectResponse {
    return response.object === "block" && "type" in response && response.type === "child_database";
}

export function richTextToString(richText: { plain_text: string }[], defaultValue: string | null = null) {
    // console.log({richText});
    return richText.length > 0 ? richText.map((text) => text.plain_text).join("") : defaultValue;
}
...
export function asTitleProperty(property: any): TitleProperty {
    ...
    throw new Error(`Property is not a title property: ${JSON.stringify(property)}`);
}
...
export function asRichTextProperty(property: any): RichTextProperty {
    ...
    throw new Error(`Property is not a rich text property: ${JSON.stringify(property)}`);
}
...
export function asNumberProperty(property: any): NumberProperty {
    ...
    throw new Error(`Property is not a number property: ${JSON.stringify(property)}`);
}
...
export function asSelectProperty(property: any): SelectProperty {
    ...
    throw new Error(`Property is not a select property: ${JSON.stringify(property)}`);
}
...
export function asDateProperty(property: any): DateProperty {
    ...
    throw new Error(`Property is not a date property: ${JSON.stringify(property)}`);
}
